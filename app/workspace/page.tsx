...
type UserPaymentProfile = { method: "kaspi_card"; recipientName: string; kaspiPhone: string; cardLast4: string; updatedByRole: "user" | "admin"; updatedAt: string };
...
function ProfileSection({ user, accountName, steamId, steamAvatar, deals, requests, paymentProfile, onPaymentProfileChange }: { user: User | null; accountName: string; steamId: string | null; steamAvatar: string | null; deals: Deal[]; requests: TradeRequest[]; paymentProfile: UserPaymentProfile | null; onPaymentProfileChange: (profile: UserPaymentProfile | null) => void }) {
  const [paymentForm, setPaymentForm] = useState({ recipientName: paymentProfile?.recipientName || "", kaspiPhone: paymentProfile?.kaspiPhone || "", cardLast4: paymentProfile?.cardLast4 || "" });
  ...
  async function savePaymentProfile(event: React.FormEvent) {
    ...
    try {
      ...
      onPaymentProfileChange(body.profile); setPaymentForm({ recipientName: body.profile.recipientName, kaspiPhone: body.profile.kaspiPhone, cardLast4: body.profile.cardLast4 }); setPaymentNotice("Kaspi payout details saved.");
  ...
  async function removePaymentProfile() {
    ...
    try {
      ...
      onPaymentProfileChange(null); setPaymentForm({ recipientName: "", kaspiPhone: "", cardLast4: "" }); setPaymentNotice("Saved payout details removed.");
    } finally { setPaymentSaving(false); }
  }
  const payoutRecords = [
