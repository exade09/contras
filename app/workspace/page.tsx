...
type UserPaymentProfile = { method: "kaspi_card"; recipientName: string; kaspiPhone: string; cardLast4: string; hasCardNumber: boolean; cardMask: string; updatedByRole: "user" | "admin"; updatedAt: string };
...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/payment-profile", { cache: "no-store" }),
    ]).then(async ([meResponse, inventoryResponse, dealsResponse, requestResponse, paymentResponse]) => {
      if (!meResponse.ok) { window.location.href = "/"; return; }
      const [me, dealData, requestData, paymentData] = await Promise.all([readJson(meResponse), readJson(dealsResponse), readJson(requestResponse), readJson(paymentResponse)]);
      setUser(me.user); setDeals(dealData.deals || []); setRequests(requestData.requests || []); setPaymentProfile(paymentData.profile || null);
  ...
  return (
    <main className="workspaceShell">
      ...
      {section === "inventory" ? (
        ...
        <ProfileSection user={user} accountName={accountName} steamId={steamId} steamAvatar={steamAvatar} deals={deals} requests={requests} paymentProfile={paymentProfile} onPaymentProfileChange={setPaymentProfile} />
      )}

      {showRequestForm && <div className="modalBackdrop"><form className="adminModal wideModal saleRequestModal" onSubmit={submitRequest}><button className="modalClose" type="button" aria-label="Close sale request form" onClick={() => setShowRequestForm(false)}>×</button><span>SALE REQUEST</span><h2>Request a manual sale review</h2><p>The administrator receives immutable snapshots of the selected owned assets and your desired amount. This action creates no Steam trade or payment.</p><div className="selectedRequestItems"><strong>{selected.size} verified inventory item(s)</strong><div>{items.filter((item) => selected.has(item.id)).map((item) => <span key={item.id}>{item.name}</span>)}</div></div><div className={`savedPayoutPreview ${paymentProfile ? "configured" : ""}`}><div><small>PAYOUT PROFILE</small><strong>{paymentProfile ? "Kaspi Bank details will be attached" : "No payout details saved"}</strong></div><span>{paymentProfile ? `${paymentProfile.recipientName}${paymentProfile.kaspiPhone ? ` · ${paymentProfile.kaspiPhone}` : ""}${paymentProfile.cardLast4 ? ` · card ending ${paymentProfile.cardLast4}` : ""}` : "Add them in Profile before or after submitting."}</span></div><div className="formGrid"><label>Desired amount<input required inputMode="decimal" value={requestForm.amount} onChange={(event) => setRequestForm({ ...requestForm, amount: event.target.value })} placeholder="0.00" /></label><label>Currency<select value={requestForm.currency} onChange={(event) => setRequestForm({ ...requestForm, currency: event.target.value })}><option>USD</option><option>EUR</option><option>RUB</option></select></label></div><label>Optional note<textarea className="shortArea" maxLength={600} value={requestForm.note} onChange={(event) => setRequestForm({ ...requestForm, note: event.target.value })} placeholder="Condition, preferred contact time, or other details" /></label>{requestError && <div className="formError" role="alert">{requestError}</div>}<button className="modalSubmit" disabled={requestSubmitting || !selected.size}>{requestSubmitting ? "Sending request…" : "Send sale request"}<b>→</b></button></form></div>}
    </main>
  );
}

function ProfileSection({ user, accountName, steamId, steamAvatar, deals, requests, paymentProfile, onPaymentProfileChange }: { user: User | null; accountName: string; steamId: string | null; steamAvatar: string | null; deals: Deal[]; requests: TradeRequest[]; paymentProfile: UserPaymentProfile | null; onPaymentProfileChange: (profile: UserPaymentProfile | null) => void }) {
  const [paymentForm, setPaymentForm] = useState({ recipientName: paymentProfile?.recipientName || "", kaspiPhone: paymentProfile?.kaspiPhone || "", cardNumber: "" });
  const [paymentSaving, setPaymentSaving] = useState(false);
  const [paymentError, setPaymentError] = useState("");
  const [paymentNotice, setPaymentNotice] = useState("");
  async function savePaymentProfile(event: React.FormEvent) {
    event.preventDefault(); setPaymentSaving(true); setPaymentError(""); setPaymentNotice("");
    try {
      const response = await fetch("/api/payment-profile", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(paymentForm) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) { setPaymentError(body.error || "Payout details could not be saved."); return; }
      onPaymentProfileChange(body.profile); setPaymentForm({ recipientName: body.profile.recipientName, kaspiPhone: body.profile.kaspiPhone, cardNumber: "" }); setPaymentNotice("Kaspi payout details saved.");
    } finally { setPaymentSaving(false); }
  }
  async function removePaymentProfile() {
    if (!confirm("Remove your saved Kaspi payout details? Existing request snapshots will remain unchanged.")) return;
    setPaymentSaving(true); setPaymentError(""); setPaymentNotice("");
    try {
      const response = await fetch("/api/payment-profile", { method: "DELETE" });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) { setPaymentError(body.error || "Payout details could not be removed."); return; }
      onPaymentProfileChange(null); setPaymentForm({ recipientName: "", kaspiPhone: "", cardNumber: "" }); setPaymentNotice("Saved payout details removed.");
    } finally { setPaymentSaving(false); }
  }
  const payoutRecords = [
  ...
  return <section className="profilePage"><div className="profileHeading"><p>ACCOUNT</p><h1>Profile</h1><span>Manage your connected identity and payout details.</span></div><div className="profileGrid"><article className="profileIdentityCard">{steamAvatar ? <SafeImage source={steamAvatar} alt={`${accountName} Steam avatar`} /> : <span>{accountName.slice(0, 1).toUpperCase()}</span>}<div><small>CLIENT PROFILE</small><h2>{accountName}</h2><p>@{user?.login || "user"}</p>{steamId && <b>Steam {steamId}</b>}</div></article><article className="profileSafetyCard"><span>✓</span><div><strong>Encrypted card storage</strong><p>Card numbers are encrypted and masked after saving. CVV, PIN and expiry date are never requested or stored.</p></div></article></div><div className="paymentSection"><div className="profileSectionTitle"><div><span>PAYOUT OPTIONS</span><h2>Payment methods</h2></div><p>You or an administrator can update the saved Kaspi payout profile.</p></div><div className="paymentMethodGrid"><article className="paymentMethodCard active"><span className="paymentMethodIcon">▣</span><div><small>CARD</small><h3>Kaspi Bank</h3><p>Recipient name, Kaspi phone and encrypted card number.</p></div><b>{paymentProfile ? "CONFIGURED" : "AVAILABLE"}</b></article><article className="paymentMethodCard"><span className="paymentMethodIcon">↔</span><div><small>BANK</small><h3>Bank transfer</h3><p>Additional bank payout options are being prepared.</p></div><b>COMING SOON</b></article><article className="paymentMethodCard"><span className="paymentMethodIcon">◇</span><div><small>WALLET</small><h3>Digital wallet</h3><p>Alternative payout methods will appear here later.</p></div><b>COMING SOON</b></article></div><form className="paymentProfileEditor" onSubmit={savePaymentProfile}><div className="paymentProfileEditorHeading"><div><small>KASPI PAYOUT PROFILE</small><h3>{paymentProfile ? "Edit saved details" : "Add payout details"}</h3></div>{paymentProfile && <span>Updated by {paymentProfile.updatedByRole}</span>}</div>{paymentProfile?.hasCardNumber && <div className="encryptedCardSummary"><span>SAVED CARD</span><strong>{paymentProfile.cardMask}</strong><small>Enter a new number below only to replace it.</small></div>}<div className="paymentProfileFields"><label>Recipient name<input required maxLength={80} autoComplete="name" value={paymentForm.recipientName} onChange={(event) => setPaymentForm({ ...paymentForm, recipientName: event.target.value })} placeholder="Recipient name" /></label><label>Kaspi phone<input inputMode="tel" maxLength={24} autoComplete="tel" value={paymentForm.kaspiPhone} onChange={(event) => setPaymentForm({ ...paymentForm, kaspiPhone: event.target.value })} placeholder="+7 700 000 00 00" /></label><label>Card number<input type="password" inputMode="numeric" minLength={13} maxLength={23} autoComplete="off" value={paymentForm.cardNumber} onChange={(event) => setPaymentForm({ ...paymentForm, cardNumber: event.target.value.replace(/[^0-9 -]/g, "") })} placeholder={paymentProfile?.hasCardNumber ? "Leave blank to keep saved card" : "0000 0000 0000 0000"} /></label></div><p className="paymentSafetyHint">The number is write-only and masked after saving. Never enter a CVV, PIN, or expiry date.</p>{paymentError && <div className="formError" role="alert">{paymentError}</div>}{paymentNotice && <div className="formSuccess" role="status">{paymentNotice}</div>}<div className="paymentProfileActions"><button className="modalSubmit" disabled={paymentSaving}>{paymentSaving ? "Saving…" : "Save payout details"}<b>→</b></button>{paymentProfile && <button className="ghostButton" type="button" disabled={paymentSaving} onClick={removePaymentProfile}>Remove saved details</button>}</div></form></div><div className="profilePayouts"><div className="profileSectionTitle"><div><span>PAYMENT REFERENCES</span><h2>Request and deal details</h2></div></div>{payoutRecords.length ? <div className="profilePayoutList">{payoutRecords.map((record) => <article key={`${record.title}-${record.id}`}><div><strong>{record.title}</strong><small>{record.details || "The administrator has selected Kaspi Bank; payout reference is pending."}</small></div><span>{record.amount}</span><b className={`statusTag ${record.status}`}>{record.status}</b></article>)}</div> : <div className="emptyProfilePayouts"><span>◎</span><div><strong>No payout references yet</strong><p>They will appear after a sale request or manual deal uses your payout profile.</p></div></div>}</div></section>;
}
