...
type User = { id: string; login: string; display_name: string; role: string; status: string; steam_id: string | null; created_at: string; last_login_at: string | null; deal_count: number; deal_total: number; payment_method: "kaspi_card" | null; payment_recipient_name: string | null; payment_kaspi_phone: string | null; payment_card_last4: string | null; payment_updated_by_role: "user" | "admin" | null; payment_updated_at: string | null };
...
export default function AdminPage() {
  ...
  const [paymentForm, setPaymentForm] = useState({ recipientName: "", kaspiPhone: "", cardLast4: "" });
  ...
  function editPaymentProfile(user: User) {
    setError(""); setPaymentUser(user); setPaymentForm({ recipientName: user.payment_recipient_name || "", kaspiPhone: user.payment_kaspi_phone || "", cardLast4: user.payment_card_last4 || "" });
  }
  async function savePaymentProfile(event: FormEvent) {
    ...
    setPaymentUser(null); flash("Kaspi payout profile saved"); await load();
  }
  ...
  return <main className="adminShell">
    ...
    {paymentUser && <div className="modalBackdrop"><form className="adminModal" onSubmit={savePaymentProfile}><button className="modalClose" type="button" onClick={() => { setPaymentUser(null); setError(""); }}>×</button><span>PAYOUT PROFILE</span><h2>Kaspi details for {paymentUser.display_name}</h2><p>The user can also update these fields from Profile. Existing request snapshots are not changed.</p><label>Recipient name<input required maxLength={80} autoComplete="off" value={paymentForm.recipientName} onChange={(event) => setPaymentForm({ ...paymentForm, recipientName: event.target.value })} placeholder="Recipient name" /></label><label>Kaspi phone<input inputMode="tel" maxLength={24} autoComplete="off" value={paymentForm.kaspiPhone} onChange={(event) => setPaymentForm({ ...paymentForm, kaspiPhone: event.target.value })} placeholder="+7 700 000 00 00" /></label><label>Card last 4<input inputMode="numeric" pattern="[0-9]{4}" maxLength={4} autoComplete="off" value={paymentForm.cardLast4} onChange={(event) => setPaymentForm({ ...paymentForm, cardLast4: event.target.value.replace(/\D/g, "") })} placeholder="1111" /></label><p className="paymentSafetyHint">Kaspi phone is preferred. Never enter a full card number, CVV, PIN, or expiry date.</p>{error && <div className="formError">{error}</div>}<button className="modalSubmit">Save payout profile <b>→</b></button>{paymentUser.payment_method && <button className="removePaymentProfileButton" type="button" onClick={removePaymentProfile}>Remove saved payout details</button>}</form></div>}
  </main>;
}
