...
type Deal = { id: string; user_id: string; login: string; display_name: string; deal_date: string; amount_cents: number; currency: string; status: string; source: string; note: string; items: string | null };
...
type TradeRequest = { id: string; user_id: string; steam_id: string; login: string | null; display_name: string | null; amount_cents: number; currency: string; status: string; note: string; created_at: string; updated_at: string; items: TradeRequestItem[] };
...
export default function AdminPage() {
  ...
  const [dealForm, setDealForm] = useState({ userId: "", dealDate: new Date().toISOString().slice(0,10), amount: "", currency: "USD", status: "completed", items: "", note: "" });
  ...
  async function createDeal(event: FormEvent) {
    ...
    if (!response.ok) { setError(body.error || "Could not create deal"); return; } setShowDealForm(false); setDealForm({ userId: "", dealDate: new Date().toISOString().slice(0,10), amount: "", currency: "USD", status: "completed", items: "", note: "" }); flash("Deal added to client history"); await load();
  }
